...
# Create basic and background plots
  ...
  hist1 <- hist(x,prob=TRUE,plot=TRUE,axes=FALSE,col=histbg,border=histborder,xlab=xlab,ylab=NULL,xlim=xlim,ylim=ylim,main=main);
  if (curve) {polygon(dens,col=tcol(bg,10),border=bg)}
  axis(1,at=hist1$mids); if (histogram | curve) {axis(2); mtext("Density",side=2,line=3)};
# Calculate polyplot statistics
  mode <- hist1$mids[which.max(hist1$counts)]
  ymode <- hist1$density[which.max(hist1$counts)]
