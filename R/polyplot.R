...
polyplot <- function(x) {
  ...
  hist1 <- hist(x, plot = TRUE, main = "Histogram ", col = "beige")
  ...
  fskew <- (sum((x - xmean)^3))
  plot(x, x, "n",
    xlab = " ", ylab = " ", ylim = c(-0.7, 3.6), xlim = c(xa - 0.1 * mm, xb + 0.1 * mm),
  ...
  segments(m2, 2, xmean + s, 1, lty = "dotted")
  segments(m1, 2, xmean - s, 1, lty = "dotted")
  segments(xmean - s, 1, mi, 0, lty = "dotted")
  segments(xmean + s, 1, ma, 0, lty = "dotted")
  segments(q3, 3, m2, 2, lty = "dotted")
  segments(q1, 3, m1, 2, lty = "dotted")
  if (fskew >= 0) {
